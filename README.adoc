...
You can start using the `*russian_phonetic*` token filter by providing analysis configuration settings:
[source,javascript]
----
...
Then you should be able to test the analyzer with `*russian_phonetic*` token filter by running the https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-analyze.html[analyze API]
[source,javascript]
----
